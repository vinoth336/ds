/* Abserve builder 
	copyright 2014 . abserve builder com 
*/

jQuery(document).ready(function($){
		
		if($.cookie("abserventheme") != '')
		{
			$('#switchTheme').attr('href',$.cookie("abserventheme"));
		} 		
		if($.cookie("abserve-sidebar") =='minimize-sidemenu'){
    ...
    $('.navbar-minimalize').click(function () {
      var w = $("body");
		...
		{
			$('#sidemenu').removeClass('expanded-menu');
			$.cookie("abserve-sidebar",'minimize-sidemenu', {expires: 365, path: '/'});
		} else {
			$('#sidemenu').addClass('expanded-menu');
			 $.cookie("abserve-sidebar",'maximaze-sidemenu', {expires: 365, path: '/'});	
		}		
    })
...
function AbserveConfirmDelete( url )
{
...
function AbserveDelete(  )
{	
	var total = $('input[class="ids"]:checkbox:checked').length;
	if(confirm('are u sure removing selected rows ?'))
	{
			$('#AbserveTable').submit();// do the rest here	
	}	
}	
