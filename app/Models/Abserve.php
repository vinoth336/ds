...
class Abserve extends Model {
	...
	public static function getRows( $args )
	{
	   ...
	   $key = with(new static)->primaryKey;
		...
		if($table == 'abserve_orders_customer'){
			/* $total = \DB::table('abserve_orders_customer')->select(['abserve_orders_customer.id'])->get();
			$total = count($total); */

			$total = \DB::select(  'SELECT abserve_orders_customer.id FROM abserve_orders_customer' . self::queryWhere(). " 
