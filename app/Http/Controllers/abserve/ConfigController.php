<?php namespace App\Http\Controllers\abserve;
...
class ConfigController extends Controller {
	...
	{	
		...
		$curInfo = \DB::table('abserve_currency')->where('base_currency',1)->first();
		$this->data['cur_symbol'] = $curInfo->symbol;
		$this->data['min_ord_val'] = \SiteHelpers::getConfig($name);
		return view('abserve.config.index',$this->data);	
	}
		...
		if (!$validator->fails()) 
		{
			...
			if(!is_null(Input::file('logo')))
			{
				...
				$file = Input::file('logo'); 
			 	$destinationPath = public_path().'/abserve/images/'; 
			...
			if($request->input('min_order_val') != ''){
				$set_name = 'min_order_value';
				$value = $request->input('min_order_val');
				$insert = \SiteHelpers::setconfig($set_name,$value);
			}
			$val  =		"<?php \n"; 
			...
			$val .= 	"define('ANDROID_VERSION','".$request->input('android_version')."');\n";
			$val .= 	"define('IOS_VERSION','".$request->input('ios_version')."');\n";	
			$val .= 	"define('CCAVENUE_ACCESSCODE','".$request->input('ccavenue_accesscode')."');\n";
			$val .= 	"define('CCAVENUE_MERCHANTID','".$request->input('ccavenue_merchantid')."');\n";
			$val .= 	"define('CCAVENUE_WORKINGCODE','".$request->input('ccavenue_workingcode')."');\n";
			$val .= 	"define('CCAVENUE_PAYMENT_STATUS','".$request->input('ccavenue_payment_staus')."');\n";
			...
			fwrite($fp,$val); 
			fclose($fp); 
			return Redirect::to('abserve/config')->with('messagetext','Setting Has Been Save Successful')->with('msgstatus','success');
		} else {
			return Redirect::to('abserve/config')->with('messagetext', 'The following errors occurred')->with('msgstatus','success')
	...
	public function getEmail()
	{
		...
		return view('abserve.config.email',$this->data);		
	...
	function postEmail( Request $request)
	{
		...
		if ($validator->passes()) 
		{
			...
			return Redirect::to('abserve/config/email')->with('messagetext', 'Email Has Been Updated')->with('msgstatus','success');	
			
		}	else {

			return Redirect::to('abserve/config/email')->with('messagetext', 'The following errors occurred')->with('msgstatus','success')
			->withErrors($validator)->withInput();
		}
	...
	public function getSecurity()
	{
		...
		return view('abserve.config.security',$this->data);		
	...
	public function postLogin( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			fwrite($fp,$val); 
			fclose($fp); 
			return Redirect::to('abserve/config/security')->with('messagetext','Setting Has Been Save Successful')->with('msgstatus','success');
		} else {
			return Redirect::to('abserve/config/security')->with('messagetext', 'The following errors occurred')->with('msgstatus','error')
			->withErrors($validator)->withInput();
		}	
	}
	...
	public function getLog( $type = null)
	{
		...
		return view('abserve.config.log',$this->data);	
	}
	...
	public function getClearlog()
	{
		...
		return Redirect::to('abserve/config/log')->with('messagetext','Cache has been cleared !')->with('msgstatus','success');	
	}
	...
	public function getTranslation( Request $request, $type = null)
	{
		...
		return view('abserve.config.translation.'.$template,$this->data);	
	}
	
	public function getAddtranslation()
	{
		return view("abserve.config.translation.create");
	} 
	...
	public function postAddtranslation( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			return Redirect::to('abserve/config/translation')->with('messagetect','New Translation has been added !')->with('msgstatus','success');	;			
			
		} else {
			return Redirect::to('abserve/config/translation')->with('messagetext','Failed to add translation !' )->with('msgstatus','error')->withErrors($validator)->withInput();
		}		
	...
	//	$filename = 'lang.php';
		...
		fwrite($fp,$form); 
		fclose($fp); 	
		return Redirect::to('abserve/config/translation?edit='.$lang.'&file='.$file)
	...
	public function getRemovetranslation( $folder )
	{
		...
		return Redirect::to('abserve/config/translation')->with('messagetext','Translation has been removed !')->with('msgstatus','success');	
