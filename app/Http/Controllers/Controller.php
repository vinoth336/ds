...
abstract class Controller extends BaseController {
	...
	public function __construct()
	{
        ...
        if(\Auth::check() == true)
        {
        	...
        	if(!\Session::get('gid'))
        	{
				...
				\Session::put('themes', 'abserve-light-blue');     		
        	}
        } 

        if(!(\Session::get('currency'))) {
			$currency = \DB::table('abserve_currency')->select('*')->where('base_currency','=',1)->first();
						...
						if(isset($f['option']['image_multiple']) && $f['option']['image_multiple'] ==1)
						{
							...
							if(!is_null(Input::file($field)))
							{
								...
								if($f['alias'] == 'abserve_deliveryboys'){
									...
									$f['option']['path_to_upload'] = '/uploads/delivery_boy/';
								}
								/*$f['option']['path_to_upload'] = ($f['option']['path_to_upload'] == '' && $f['alias'] == 'abserve_restaurants') ? '/uploads/hotels/' : $f['option']['path_to_upload'];*/
	...
	public function getSearch()
	{
		...
		return view('abserve.module.utility.search',$this->data);
