    ...
    */
    ...
    'grant_types' => [
        'password' => [
             ...
             'callback' => function($username, $password, $user_type){
                $credentials = [
                ...
                if($user_type =='user'){
                  ...
                  $row = \DB::table('abserve_deliveryboys')->select('*')->where('phone_number','=',$username)->get();
                  $password_match = Hash::check($password, $row[0]->password);
                  if ($password_match == 1 && $row[0]->phone_number == $username) {
                        return $row[0]->id;
                  }else{
                     return false;
                  }
