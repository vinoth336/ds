       ...
       <ul id="sidemenu" class="nav expanded-menu">
		...
		<li class="nav-header">
			<div class="dropdown profile-element" style="text-align:center;"> <span>
				...
				<span class="clear"> <span class="block m-t-xs"> <strong class="font-bold">{{ \SiteHelpers::getuname(Auth::id()) }}</strong>
				 <br />
				{!! Lang::get('core.lastlogin') !!} :
				<small>{{ date("F j, Y", strtotime(Session::get('ll'))) }}</small>				
				 </span> 
				 </span>
				 </a>
			</div>
		...
		<li class="hidden-xs"><a href="javascript:void(0)" class="navbar-minimalize ">
	      <i class="fa fa-bars"></i><span class="nav-label">{!! trans('core.abs_collapse_menu') !!}</span></a>
      	</li>
