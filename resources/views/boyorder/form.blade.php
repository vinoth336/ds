...
<div class="sbox animated fadeInRight">
	...
	<div class="sbox-content"> 	
		 ...
		 {!! Form::open(array('url'=>'boyorder/save?return='.$return, 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) !!}
<div class="col-md-12">
						<fieldset><legend> {!! trans('core.abs_cashondeliveryorder') !!}</legend>
			...
			<div style="clear:both"></div>	
				  ...
				  <div class="form-group">
					...
					<button type="button" onclick="location.href='{{ URL::to('boyorder?return='.$return) }}' " class="btn btn-success btn-sm "><i class="fa  fa-arrow-circle-left "></i>  {!! Lang::get('core.sb_cancel') !!} </button>
					</div>	  
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#boy_id").jCombo("{{ URL::to('boyorder/comboselect?filter=abserve_deliveryboys:id:username') }}",
		{  selected_value : '{{ $row["boy_id"] }}' });
		
		$("#orderid").jCombo("{{ URL::to('boyorder/comboselect?filter=abserve_orders:id:id') }}",
		{  selected_value : '{{ $row["orderid"] }}' });
		
		$("#partner_id").jCombo("{{ URL::to('boyorder/comboselect?filter=tb_users:id:username') }}",
