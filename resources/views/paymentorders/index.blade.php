...
<div class="sbox-tools" >
	...
	<div class="sbox-content"> 	
	    <div class="toolbar-line ">
			<!--@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('paymentorders/update') }}" class="tips btn btn-sm btn-white"  title="{!! Lang::get('core.btn_create') !!}">
			...
			@endif--> 
			<!--<a href="{{ URL::to( 'paymentorders/search') }}" class="btn btn-sm btn-white" onclick="AbserveModal(this.href,'Advance Search'); return false;" ><i class="fa fa-search"></i> {!! trans('core.btn_search') !!}</a>				
			@if($access['is_excel'] ==1)
			<a href="{{ URL::to('paymentorders/download?return='.$return) }}" class="tips btn btn-sm btn-white" title="{!! Lang::get('core.btn_download') !!}">
	 ...
	 {!! Form::open(array('url'=>'paymentorders/delete/', 'class'=>'form-horizontal' ,'id' =>'AbserveTable' )) !!}
        ...
        <tbody>        						
            @foreach ($rowData as $row)
						 ...
						 <td>					 
							...
							@elseif($field['field'] =='status')								
								...
								@elseif($row->status == 11)
									<span class="label status label-danger">Order Returned</span>
                                @endif
				...
				<!--  <td>
						...
						<a href="{{ URL::to('paymentorders/show/'.$row->id.'?return='.$return)}}" class="tips btn btn-xs btn-primary" title="{!! Lang::get('core.btn_view') !!}"><i class="fa  fa-search "></i></a>
						@endif
						@if($access['is_edit'] ==1)
						<a  href="{{ URL::to('paymentorders/update/'.$row->id.'?return='.$return) }}" class="tips btn btn-xs btn-success" title="{!! Lang::get('core.btn_edit') !!}"><i class="fa fa-edit "></i></a>
						@endif
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#AbserveTable').attr('action','{{ URL::to("paymentorders/multisearch")}}');
		$('#AbserveTable').submit();
	});
