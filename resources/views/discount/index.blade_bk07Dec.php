            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 @foreach ($tableGrid as $field)
					 @if($field['view'] =='1')
						 ...
						 <td>					 
                            ...
                            @elseif($field['field'] =='coupon_use_type')								
								@if($row->coupon_use_type == 1)
                                	Single
                                @else
                                	Multiple
                                @endif
                            @elseif($field['field'] =='offer_type')								
								@if($row->offer_type == 1)
                                	Free Delivery
                                @else
                                	Offer
				 ...
				 <td>
						...
						<a href="{{ URL::to('discount/show/'.$row->id.'?return='.$return)}}" class="tips btn btn-xs btn-primary" title="{!! Lang::get('core.btn_view') !!}"><i class="fa  fa-search "></i></a>
						@endif
						@if($access['is_edit'] ==1)
						<a  href="{{ URL::to('discount/update/'.$row->id.'?return='.$return) }}" class="tips btn btn-xs btn-success" title="{!! Lang::get('core.btn_edit') !!}"><i class="fa fa-edit "></i></a>
						@endif
