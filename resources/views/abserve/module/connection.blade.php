...
{!! Form::open(array('url'=>'abserve/module/conn/'.$module_name, 'id'=>'conn_form','class'=>'form-vertical' ,'parsley-validate'=>'','novalidate'=>' ')) !!}
	<div id="result"></div>
...
$(document).ready(function(){
	...
	$("#db").jCombo("{{ URL::to('abserve/module/combotable') }}",
	{ selected_value : "{{ $f['db'] }}" });

	
	<?php $display = explode("|", $f['display']); ?>

	$("#key").jCombo("{{ URL::to('abserve/module/combotablefield') }}?table=",
	{ selected_value : "{{ $f['key'] }}", parent: "#db", initial_text : ' Primary Key' });		
	$("#lookup_value1").jCombo("{{ URL::to('abserve/module/combotablefield') }}?table=",
	{ selected_value : "<?php echo (isset($display[0]) ? $display[0] : '');?>", parent: "#db",   initial_text : ' Display Text 1'});
	$("#lookup_value2").jCombo("{{ URL::to('abserve/module/combotablefield') }}?table=",
	{ selected_value : "<?php echo (isset($display[1]) ? $display[1] : '');?>", parent: "#db",   initial_text : ' Display Text 2'});
	$("#lookup_value3").jCombo("{{ URL::to('abserve/module/combotablefield') }}?table=",
