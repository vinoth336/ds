...
@section('content')
<script type="text/javascript" src="{{ asset('abserve/js/plugins/jquery.nestable.js') }}"></script>
  ...
  <div class="page-content row">
	...
	<ul class="nav nav-tabs" style="margin:10px 0;">
		<li @if($active == 'top') class="active" @endif ><a href="{{ URL::to('abserve/menu?pos=top')}}"><i class="icon-paragraph-justify2"></i> {!! Lang::get('core.tab_topmenu') !!} </a></li>
		<li @if($active == 'sidebar') class="active" @endif><a href="{{ URL::to('abserve/menu?pos=sidebar')}}"><i class="icon-paragraph-justify2"></i> {!! Lang::get('core.tab_sidemenu') !!}</a></li>	
	</ul>  	
 ...
 <div class="infobox infobox-info fade in">
  ...
  <p> {!! Lang::get('core.t_tipsdrag') !!}</p>	
			...
			@foreach ($menus as $menu)
				  <li data-id="{{$menu['menu_id']}}" class="dd-item dd3-item">
					<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{$menu['menu_name']}}
						...
						<a href="{{ URL::to('abserve/menu/index/'.$menu['menu_id'].'?pos='.$active)}}"><i class="icon-cogs"></i></a></span>
					</div>
							...
							@foreach ($menu['childs'] as $menu2)
							 <li data-id="{{$menu2['menu_id']}}" class="dd-item dd3-item">
								<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{$menu2['menu_name']}}
									<span class="pull-right">
									<a href="{{ URL::to('abserve/menu/index/'.$menu2['menu_id'].'?pos='.$active)}}"><i class="icon-cogs"></i></a></span>
								</div>
									...
									@foreach($menu2['childs'] as $menu3)
									 	<li data-id="{{$menu3['menu_id']}}" class="dd-item dd3-item">
											<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{ $menu3['menu_name'] }}
												<span class="pull-right">
												<a href="{{ URL::to('abserve/menu/index/'.$menu3['menu_id'].'?pos='.$active)}}"><i class="icon-cogs"></i></a>
												</span>
											</div>
										</li>	
									@endforeach
		 ...
		 {!! Form::open(array('url'=>'abserve/menu/saveorder/', 'class'=>'form-horizontal','files' => true)) !!}	
 ...
 <p> {!! Lang::get('core.t_tipsnote') !!}	</p>
		...
		<h5>
		 ...
		 {!! Form::open(array('url'=>'abserve/menu/save/', 'class'=>'form-horizontal','files' => true)) !!}
				<div class=" box">	
			  ...
			  <div class="form-group">
				...
				@if($row['menu_id'] !='')
					<button type="button"onclick="AbserveConfirmDelete('{{ URL::to('abserve/menu/destroy/'.$row['menu_id'])}}')" class="btn btn-danger ">  Delete </button>
				@endif	
				</div>	  
